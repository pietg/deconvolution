	...
	#sourceCpp("nonparMLE_unif_IQM.cpp")
	
	NumIt = 10000
	n = 1000
	step=0.1

	upper_bound = 2	# upper bound for truncated exponential
	ngrid = 19
	
	low = 0.5		# lower bound for E
	high = 1.5		# upper bound for E
...
# Compute MLE	
	...
	for (i in 1: ngrid)
	{
		y1[i] <- 0.263555964*(4*f0(i*step)*F0(i*step)*(1-F0(i*step)))^(2/3)
		h <- function(e){e^(-1)*(high-low)^(-1)*(1/(F0(i*step)-F0(i*step-e))+1/(F0(i*step+e)-F0(i*step)))}
		a <- integrate(Vectorize(h), lower = low, upper = high)$value
		#a <- quad(Vectorize(h),1/2,3/2)
		y0[i] <- 0.263555964*(4*f0(i*step)/a)^(2/3)
		#y0[i] <- 0.263555964*(4*f0(i* step)*F0(i* step)*(1-F0(i* step)))^(2/3)
		#y0[i] <- 0.263555964*(4*f0(i*step)/(1/(F0(i*step)-F0(i*step-1))+1/(F0(i*step+1)-F0(i*step))))^(2/3)
		y[i] <-  variances[i,2]
	}
